...
| **S13** | Low | Final Docker image runs as root (no `USER`). | Container breakout has root inside the image. | **Mitigated** | Dockerfile now creates a `curatorx` user (UID/GID 1000) and switches via `USER curatorx`. Bind-mount volumes for `/config` must be owned by 1000:1000. |
