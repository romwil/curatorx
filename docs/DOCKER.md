...
Deploy CuratorX as a single container with a persistent `/config` volume for `settings.json` and `curatorx.db`. Everyday tag: **`romwil/curatorx:latest`** (CA default). Pin **`:1.8`** or **`:1.8.13`** when you need a fixed line or exact build.
...
| **Image** | `romwil/curatorx:latest` (or `:1.8` / `:1.8.13`) — multi-arch amd64+arm64 |
...
./rollout.sh           # :latest
./rollout.sh 1.8.13    # pin a release tag
...
**Pin a release** when you want to avoid floating `:latest`: set Repository to `romwil/curatorx:1.8.13` (or `:1.8`) in the template, then pull that tag. Line tags (`:1.8`) still float within the minor line.
...
./scripts/docker-release.sh 1.8.13 --also-line 1.8
./scripts/docker-release.sh 1.8.13 --date-tag # also :latest-YYYYMMDD (CA testing)
```
