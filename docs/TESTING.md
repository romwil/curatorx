...
Multi-role GUI / auth-ON sidecar tooling is **maintainer-local only** (not in this repo). Keep personal runbooks on the host deploy kit (e.g. automat `curatorx-qa-scripts`); do not turn production auth off or mount production `/config` for role testing.
