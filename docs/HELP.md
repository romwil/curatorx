...
The small horizontal rails on the Explore hub and Plot Lab’s **Surprising neighbors**, plus a person’s credit-annotated filmography, intentionally stay context-dense rather than repeating a full browse toolbar in every short embedded strip. All full walls—including a deep link from a title’s genre, cast, director, decade, language, or country—use the shared controls, Poster/List results, CSV menu, and card grip. The short rails still use the same card/grip actions; choose the linked full Explore section, Tag, Plot, or facet wall when you need filters, List view, or CSV.
