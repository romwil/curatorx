...
class Database:
        ...
        with self.connect() as conn:
            conn.execute(
                ...
                INSERT INTO curator_persona_metrics (
                    metric_id, curator_name, val_bro_prof, val_dipl_snark, val_pass_auto, last_modified
                ) VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    ...
    def ensure_chat_session(self, session_id: str, lens_id: str = DEFAULT_LENS_ID) -> None:
        now = time.time()
        resolved = lens_id or DEFAULT_LENS_ID
        with self.connect() as conn:
            conn.execute(
                """
                INSERT OR IGNORE INTO chat_sessions (id, created_at, updated_at, lens_id)
                VALUES (?, ?, ?, ?)
                """,
                (session_id, now, now, resolved),
            )
            conn.execute(
                "UPDATE chat_sessions SET lens_id = ?, updated_at = ? WHERE id = ?",
